...
pair's kinship coefficient is set to 0.5.
The case of triple monozygotes makes the code a bit harder.

Because we don't want to put any computation into the middle of the for loops
the focus is on preprocessing.
Create a matrix containing the mz pairs, and then manufacture an index
vector which contains, for all members of a set, the lowest id for that
set.  The value for non-mz subjects is arbitrary.
So for instance assume the following rather difficult example.
\begin{verbatim}
   1  2
   1  3
   5  6
...
monzygotic pairs.  
The vector \texttt{mzgrp} will be 1, 1, 1, 4, 5, 5, 1, 8, 9, 9,
of which elements 4 and 8 are not part of any set, and will
be ignored.
From this we create a list of 
matrices \texttt{matlist} whose first element is a matrix
containing all possible
pairings of the four elements 1, 2, 3, and 7 (12 rows); the second is
the matrix with rows of 4,5 and 5,4; and the third is the matrix for
the 9, 10 pair.
These are the elements of kmat that will have to be overridden to a
the value of self-self kinship for the set.
This will usually be 0.5, unless there is inbreeding of the
twinset's ancestors.
We need to do this reset at each level of the outer loop, before
descendents of any of the twinsets are computed.  
(Technically, we only need to reset for levels on which a
twinset appears, but since there are normally only a few levels
